...
- For generated files (protobuf, lockfiles, MODULE.bazel), accept theirs during the cherry-pick to unblock the cherry-pick, but the result is almost certainly wrong for the target branch. The "Generated files" warning section in the PR body tells the human reviewer to regenerate and commit them before merge. Do not describe these files as "regenerated by the build" in conflict-detail lines — Bazel/protoc/etc. only update them in the local workspace; CI does NOT push a corrected version back to the PR, so the file must be committed manually.
